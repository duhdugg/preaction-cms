...
model.Meta = sequelize.define('meta', {
  /**
   * @memberof lib/db.model.Meta
   * @type {string}
   */
  key: { type: Sequelize.STRING },
  /**
...
const sync = async () => {
  ...
  await model.Meta.sync({ force }).then(async () => {
    if (
      env.nodeEnv !== 'test' &&
      env.nodeEnv !== 'upgrade-db' &&
      !cache.store['#dbVersion']
    ) {
      const dbMeta = await model.Meta.findOrCreate({
        where: { key: 'db' },
        defaults: { value: { version: pkg.version } },
      })
      if (dbMeta) {
        cache.set('#dbVersion', dbMeta[0].value.version)
        console.debug({ dbVersion: cache.store['#dbVersion'].contents })
      }
    }
  })
  return
