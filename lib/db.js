...
// set up the expressModule which the main express application
...
expressModule
  ...
  .post(adminRequired, async (req, res) => {
...
const restoreBackup = async (filename) => {
  ...
  console.debug('calculating sha256 hash')
  let hash = await hasha.fromFile(`data/backups/${filename}`, {
    algorithm: 'sha256',
  })
  console.debug('copying file')
  await copyFile(`data/backups/${filename}`, 'data/db.sqlite')
  console.debug('file copied')
  let verifiedHash = await hasha.fromFile('data/db.sqlite', {
    algorithm: 'sha256',
  })
