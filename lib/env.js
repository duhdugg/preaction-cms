...
module.exports = {
   ...
   * @example CMS_COOKIE_SECRET="$(dd if=/dev/random status=none | strings | sed 's/[^a-zA-Z0-9]//g' | tr -d '\n' | dd bs=1 count=43 status=none)" npm start
   */
  ...
  /**
   * @type {boolean}
   * @description allows removing EXIF data from uploads.
   * Defaults to `true`.
   * Set with the `CMS_DO_NOT_REMOVE_EXIF` environment variable
   * @example CMS_DO_NOT_REMOVE_EXIF=1 npm start
   */
  removeExif: process.env.CMS_DO_NOT_REMOVE_EXIF ? false : true,
  /**
   * @type {string}
  ...
  /**
   * @type {boolean}
   * @description enables server-side rendering
   * On by default.
   * Disable with the `CMS_SSR_DISABLE` environment variable.
   * @example CMS_SSR_DISABLE=1 npm start
   */
  ssr: process.env.CMS_SSR_DISABLE ? false : true,
}
