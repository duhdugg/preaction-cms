...
const createHero = (parentId) => {
  return model.Page.findOrCreate({
    where: { key: 'hero', parentId },
    defaults: { pageType: 'content', parentId, userCreated: false },
  })
}
...
// 404 pages will still need to use applied settings and sitemap
 ...
 * @param {boolean} excludeHeaderFooterHero
...
const getFullPageById = async (
  ...
  // headers, footers, and heroes don't need these extra attributes
  ...
  let pt = await getPageTree(page, depth, excludeHeaderFooterHero)
...
const getPageTree = async (page, depth = 0, excludeHeaderFooterHero = true) => {
...
// ensures that a default home, header, footer, and hero page are created
const findOrCreateDefaultPages = async () => {
  ...
  const hero = await createHero(home.id)
  return { home, header, footer, hero }
}
 ...
 * @returns {Promise} `Object` with default home, header, footer, and hero pages
 */
const sync = async () => {
  await dbSync()
...
middleware
  ...
  .get(cache.middleware, async (req, res) => {
    let settings
    try {
      const page = await getPageByPath(req.params[0])
      settings = await getAppliedPageSettings(page.id)
      cache.set(req.path, settings, 'application/json')
    } catch (error) {}
    if (!settings && env.cache) {
      const splitPath = req.params[0].split('/').filter((s) => !!s)
      for (let x = 1; x < splitPath.length; x++) {
        const path = splitPath.slice(0, splitPath.length - x).join('/')
        const closestCache = cache.store[`/api/page/settings/by-key/${path}/`]
        if (closestCache) {
          settings = closestCache.contents
        }
      }
      if (!settings) {
        settings = cache.store['/api/page/settings/by-key/home/'].contents
      }
    } else if (!settings && !env.cache) {
      const page = getClosestPageByPath(req.params[0])
      settings = await getAppliedPageSettings(page.id)
    }
    res.json(settings)
  })
...
middleware
  ...
  .get(cache.middleware, async (req, res) => {
    ...
    try {
      const page = await getPageByPath(req.params[0])
      siteMap = await getSiteMap(page)
      cache.set(req.path, siteMap, 'application/json')
    } catch (error) {}
    if (!siteMap && env.cache) {
      const splitPath = req.params[0].split('/').filter((s) => !!s)
      for (let x = 1; x < splitPath.length; x++) {
        const path = splitPath.slice(0, splitPath.length - x).join('/')
        const closestCache = cache.store[`/api/page/sitemap/by-key/${path}/`]
        if (closestCache) {
          siteMap = closestCache.contents
        }
      }
      if (!siteMap) {
        siteMap = cache.store['/api/page/settings/by-key/home/'].contents
      }
    } else if (!siteMap && !env.cache) {
      const page = getClosestPageByPath(req.params[0])
      siteMap = await getSiteMap(page)
    }
    res.json(siteMap)
