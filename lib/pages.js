...
middleware.use('/api/page/by-key/', async (req, res) => {
  try {
    const page = await getFullPageByPath(req.path, 2)
    res.json(page)
...
middleware.use('/api/page/settings/by-key/', async (req, res) => {
  try {
    const page = await getClosestPageByPath(req.path)
...
middleware.use('/api/page/sitemap/by-key/', async (req, res) => {
  try {
    const page = await getClosestPageByPath(req.path)
