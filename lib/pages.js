...
const getFullPageById = async (pageId, depth) => {
  ...
  const cleanSmChild = (pg) => {
    delete pg.createdAt
    delete pg.id
    delete pg.pageType
    delete pg.parentId
    delete pg.updatedAt
    delete pg.userCreated
    pg.settings = {
      includeInNav: pg.settings.includeInNav,
      navOrdering: pg.settings.navOrdering,
    }
    for (let child of pg.children) {
      cleanSmChild(child)
    }
  }
  for (let child of sm.children) {
    cleanSmChild(child)
  }
  let pt = await getPageTree(page, depth)
  // remove attributes not needed in pageTree
  pt = {
    children: pt.children,
  }
  const cleanPtChild = (pg) => {
    delete pg.createdAt
    delete pg.id
    delete pg.key
    delete pg.pageType
