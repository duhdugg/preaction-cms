...
const copyPage = async (fullPage, destinationParentId, destinationKey) => {
  ...
  for (const block of fullPage.blocks) {
...
const getFullPageById = async (pageId, depth) => {
  ...
  const sm = await getSiteMap(page)
  const pt = await getPageTree(page, depth)
...
const getFullPageByPath = async (path) => {
  const page = await getPageByPath(path)
  return await getFullPageById(page.id)
}
...
const getPageTree = async (page, depth) => {
  ...
  pageTree.path = path
  const results = await model.Page.findAll({ where: { parentId: page.id } })
  if (results) {
...
middleware
  ...
  .put(adminRequired, csrf.protect, async (req, res) => {
    ...
    const newKey = req.body.key.toLowerCase().trim()
    if (page.key !== newKey) {
      if (!page.parentId) {
        switch (newKey) {
          case 'home':
  ...
  .delete(adminRequired, csrf.protect, async (req, res) => {
    ...
    await model.Page.destroy({ where: { id: req.params.id } })
    await model.Page.destroy({ where: { parentId: req.params.id } })
    db.backup()
    res.json(page)
    cache.clear()
  })
...
middleware
  .route('/api/page/by-key/:key')
  .get(cache.middleware, async (req, res) => {
    try {
      const page = await getFullPageByPath(req.params.key, 2)
      res.json(page)
...
middleware
  .route('/api/page/by-key/:parentKey/*')
  .get(cache.middleware, async (req, res) => {
    try {
      const p = `${req.params.parentKey}/${req.params[0]}`
      const page = await getFullPageByPath(p, 2)
      res.json(page)
      cache.set(req.url, page, 'application/json')
    } catch (error) {
      res.status(404).json({ error })
...
middleware
  .route('/api/page/settings/by-key/:key')
  .get(cache.middleware, async (req, res) => {
    try {
      const page = await getClosestPageByPath(req.params.key)
...
middleware
  ...
  .get(cache.middleware, async (req, res) => {
    try {
      const p = `${req.params.parentKey}/${req.params[0]}`
      const page = await getClosestPageByPath(p)
      const settings = await getAppliedPageSettings(page.id)
      res.json(settings)
      cache.set(req.url, settings, 'application/json')
    } catch (error) {
...
middleware
  .route('/api/page/sitemap/by-key/:key')
  .get(cache.middleware, async (req, res) => {
    try {
      const page = await getClosestPageByPath(req.params.key)
...
middleware
  .route('/api/page/sitemap/by-key/:parentKey/*')
  .get(cache.middleware, async (req, res) => {
    try {
      const p = `${req.params.parentKey}/${req.params[0]}`
      const page = await getClosestPageByPath(p)
...
middleware
  ...
  .delete(adminRequired, csrf.protect, async (req, res) => {
    ...
    const pageBlocks = await model.PageBlock.findAll({
      where: { ordering: { $gt: ordering }, pageId },
    })
...
middleware
  ...
  .delete(adminRequired, csrf.protect, async (req, res) => {
    ...
    const contents = await model.PageBlockContent.findAll({
      where: { ordering: { $gt: ordering }, pageblockId },
    })
