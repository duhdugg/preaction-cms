...
const getFullPageById = (pageId, depth) => {
  return new Promise((resolve, reject) => {
    model.Page.findByPk(pageId, {
      ...
      getAppliedPageSettings(page.parentId).then((fallbackSettings) => {
        ...
        getSiteMap(page).then((sm) => {
          getPageTree(page, depth).then((pt) => {
            obj.siteMap = sm
            obj.tree = pt
            resolve(obj)
          })
...
const getSiteMap = (page) => {
  return new Promise((resolve, reject) => {
    let resolved = false
    getPageTree(page, 3).then((pageTree) => {
      if (pageTree.settings.site) {
        resolve(pageTree)
        resolved = true
      } else {
        // get the nearest "site" page
        getAncestry(page).then((ancestry) => {
          ancestry.reverse()
          ancestry.forEach((pg) => {
            if (pg.settings.site && !resolved) {
              resolve(getPageTree(pg, 3))
              resolved = true
            }
          })
          if (!resolved) {
    ...
    getPagePath(page).then((path) => {
      ...
      model.Page.findAll({
        ...
        if (results) {
          ...
          if (!depth || depth > 1) {
            results.forEach((pg) => {
              getPageTree(pg, depth ? depth - 1 : undefined)
                .then((pt) => {
...
middleware.route('/api/page/by-key/:key').get(cache.middleware, (req, res) => {
  getPageByPath(req.params.key)
    .then((page) => {
      getFullPageById(page.id, 2).then((page) => {
        res.json(page)
