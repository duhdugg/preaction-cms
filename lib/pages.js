...
model.PageBlock = db.sequelize.define('pageblock', {
   ...
   * @instance
   * @type {string}
   * @description PageBlock.blockType should be 'content', 'ext', 'iframe', or 'nav'
   */
 ...
 * @param {boolean} excludeHeaderFooter
 * if true, the header and footer will be excluded from the tree
 * @returns {Promise} `Object` "full" page, meaning blocks, content,
 * tree, and sitemap included
 */
const getFullPageById = async (pageId, depth, excludeHeaderFooter = true) => {
  const page = await model.Page.findByPk(pageId, {
    include: [
      {
  ...
  let pt = await getPageTree(page, depth, excludeHeaderFooter)
...
const getPageTree = async (page, depth = 0, excludeHeaderFooter = true) => {
 ...
 * @returns {Promise} `Object` with default home, header, and footer pages
 */
const sync = async () => {
  await dbSync()
