...
// PageBlock.blockType should be 'content', 'iframe', or 'nav'
...
expressModule
  ...
  .post(session.adminRequired, (req, res) => {
...
expressModule
  ...
  .put(session.adminRequired, (req, res) => {
  ...
  .delete(session.adminRequired, (req, res) => {
    model.Page.findByPk(req.params.id).then(page => {
      if (!page) {
        res.status(404).json({ error: 'not found' })
        return
