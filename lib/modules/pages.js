...
let getSiteMap = page => {
  return new Promise((resolve, reject) => {
    let resolved = false
    getSiteTree(page).then(siteTree => {
      if (siteTree.settings.site) {
        resolve(siteTree)
        resolved = true
      } else {
        getAncestry(page).then(ancestry => {
          ...
          if (!resolved) {
            model.Page.findOne({
              ...
              model.Page.findAll({
                ...
                if (count) {
                  pages.forEach(pg => {
                    getSiteTree(pg).then(st => {
                      siteMap.children.push(st)
                      if (
...
let getSiteTree = page => {
  let siteTree = JSON.parse(JSON.stringify(page))
  siteTree.children = []
  return new Promise((resolve, reject) => {
      ...
      model.Page.findAll({
        ...
        if (results) {
          ...
          results.forEach(pg => {
            getSiteTree(pg)
              .then(st => {
                siteTree.children.push(st)
                i++
              })
              .then(() => {
                if (i >= count) {
                  siteTree.children.sort((a, b) => {
                    let c = 0
                    if (a.title > b.title) {
                      c++
          ...
          return createHeader(page.id)
            .then(results => {
              ...
              createFooter(page.id).then(results => {
                let footer = results[0]
                db.model.Settings.findOne({ where: { key: 'headerId' } }).then(
                  headerSetting => {
                    headerSetting.value = header.id
                    headerSetting.save().then(() => {
                      db.model.Settings.findOne({
                        where: { key: 'footerId' }
                      }).then(footerSetting => {
                        footerSetting.value = footer.id
...
expressModule
  ...
  .post(session.authenticationRequired, (req, res) => {
    ...
    let settings = {}
    if (req.body.blockType === 'content') {
      settings.center = false
      settings.header = ''
      settings.headerLevel = 0
      settings.lgWidth = 12
      settings.mdWidth = 12
      settings.padding = 0
      ...
      settings.smWidth = 12
      settings.xsWidth = 12
    } else if (req.body.blockType === 'nav') {
      settings.header = ''
...
expressModule
  ...
  .post(session.authenticationRequired, (req, res) => {
    ...
    let settings = {}
    if (req.body.contentType === 'wysiwyg') {
      settings.xsWidth = 12
      settings.smWidth = 12
