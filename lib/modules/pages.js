...
expressModule.route('/api/page').post(adminRequired, (req, res) => {
...
expressModule.route('/api/page/:id/blocks').post(adminRequired, (req, res) => {
...
expressModule.route('/api/page/:id/settings').get(adminRequired, (req, res) => {
  model.Page.findByPk(req.params.id).then((page) => {
    if (!page) {
      res.status(404).json({ error: 'not found' })
      return
...
expressModule.route('/api/page/:id/sitemap').get(adminRequired, (req, res) => {
  model.Page.findByPk(req.params.id).then((page) => {
    getSiteMap(page).then((siteMap) => {
      res.json(siteMap)
    })
