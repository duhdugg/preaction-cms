...
const clientRender = require('../build/static/js/client.cjs.js').render
const db = require('./db.js')
const env = require('./env.js')
const { sanitizeContent } = require('./pages.js')
  ...
  renderClient: async (req, res, options = {}) => {
    ...
    fs.readFile('build/index.html', 'utf8', (err, contents) => {
      ...
      contents = contents
        ...
        .replace(
          '<div id="root"></div>',
          '<div id="root">' + clientRender(req.path, options.page) + '</div>'
        )
      res.send(contents)
