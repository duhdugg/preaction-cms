...
test('exports funcs', () => {
  expect(typeof pages.copyPage).toBe('function')
  expect(typeof pages.copyPageWithChildren).toBe('function')
  expect(typeof pages.getAppliedPageSettings).toBe('function')
  expect(typeof pages.getFullPageById).toBe('function')
  expect(typeof pages.getPageByPath).toBe('function')
  expect(typeof pages.getPagePath).toBe('function')
})
test('exports model', () => {
...
test('default pages are created', async () => {
  ...
  const jumboResponse = await unauth.get('/api/page/by-key/home/jumbo/')
  ...
  expect(jumboResponse.statusCode).toBe(200)
})
...
test('update created page', async () => {
  ...
  const updateResponse = await auth.put(`/api/page/5?token=${token}`).send(page)
...
test('add block to page', async () => {
  const { auth, token, unauth } = await client.init()
  let response
  response = await auth.post(`/api/page/5/blocks?token=${token}`).send({
    blockType: 'content',
  })
  expect(response.statusCode).toBe(200)
  ...
  response = await auth
    .post(`/api/page/5/blocks?token=${token}`)
  ...
  response = await auth
    ...
    .send({ blockType: 'ext' })
  expect(response.statusCode).toBe(200)
  expect(response.body.blockType).toBe('ext')
  ...
  expect(response.body.pageblocks[3].blockType).toBe('ext')
})
...
test('/api/page/:id/settings', async () => {
  const { auth } = await client.init()
  const response = await auth.get('/api/page/5/settings')
  expect(response.statusCode).toBe(200)
})
test('/api/page/:id/sitemap', async () => {
  const { auth } = await client.init()
  const response = await auth.get('/api/page/5/sitemap')
  expect(response.statusCode).toBe(200)
})

test('deep copy page', async () => {
  jest.setTimeout(30000)
  const page = await pages.getFullPageByPath('/test00/')
  const newPage = await pages.copyPageWithChildren(page, null, 'test3')
...
describe('/api/page error responses', () => {
  ...
  test('no reserved keys', async () => {
    ...
    response = await auth.post(`/api/page?token=${token}`).send(body)
    expect(response.statusCode).toBe(400)
    expect(response.body.error).toBe('jumbo is reserved')
    body.key = 'home'
    response = await auth.post(`/api/page?token=${token}`).send(body)
    expect(response.statusCode).toBe(400)
...
describe('/api/page/:id error responses', () => {
  test('get method not allowed', async () => {
    const { unauth } = await client.init()
    const response = await unauth.get('/api/page/5')
    expect(response.statusCode).toBe(404)
  })
  test('admin required for PUT method', async () => {
    const { unauth } = await client.init()
    const response = await unauth.put('/api/page/5').send({})
    expect(response.statusCode).toBe(403)
    expect(response.body.error).toBe('forbidden')
  })
  test('admin required for DELETE method', async () => {
    const { unauth } = await client.init()
    const response = await unauth.delete('/api/page/5')
    expect(response.statusCode).toBe(403)
    expect(response.body.error).toBe('forbidden')
  })
  test('csrf protection for PUT method', async () => {
    const { auth } = await client.init()
    const response = await auth.put('/api/page/5').send({})
    expect(response.statusCode).toBe(403)
    expect(response.body.error).toBe('invalid csrf token')
  })
  test('csrf protection for DELETE method', async () => {
    const { auth } = await client.init()
    const response = await auth.delete('/api/page/5')
    expect(response.statusCode).toBe(403)
    expect(response.body.error).toBe('invalid csrf token')
  })
  ...
  test('no reserved keys on PUT method', async () => {
    ...
    response = await auth.put(`/api/page/5?token=${token}`).send(body)
    expect(response.statusCode).toBe(400)
    expect(response.body.error).toBe('login is reserved')
    body.key = 'header'
    response = await auth.put(`/api/page/5?token=${token}`).send(body)
  ...
  test('no existing keys on PUT method', async () => {
    const { auth, token } = await client.init()
    const response = await auth.put(`/api/page/5?token=${token}`).send({
      pageType: 'content',
      key: 'test1',
      title: 'Title',
    })
...
describe('/api/page/:id/settings error responses', () => {
  test('admin required', async () => {
    const { unauth } = await client.init()
    const response = await unauth.get('/api/page/5/settings')
    expect(response.statusCode).toBe(403)
    expect(response.body.error).toBe('forbidden')
  })
...
describe('/api/page/:id/sitemap error responses', () => {
  test('admin required', async () => {
    const { unauth } = await client.init()
    const response = await unauth.get('/api/page/5/sitemap')
    expect(response.statusCode).toBe(403)
    expect(response.body.error).toBe('forbidden')
  })
  test('404 on invalid id', async () => {
